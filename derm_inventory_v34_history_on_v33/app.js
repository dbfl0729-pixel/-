...
function renderHistory() {
  const card = el('div', { class: 'card' });
  card.appendChild(el('h2', { text: '재고 히스토리' }));
  ...
  [
    ['all', '전체'],
    ['inbound', '일반 입고'],
    ['keep_purchase', '선결제 구매'],
    ['keep_ship', '선결제 차감'],
    ['daily', '일일재고 저장']
  ].forEach(([v, t]) => typeSel.appendChild(el('option', { value: v, text: t })));

  card.appendChild(el('div', { class: 'row top' }, [
    field('월 선택', monthInput),
    field('구분', typeSel),
  ]));

  const wrap = el('div', { class: 'table-wrap', style: { marginTop: '16px' } });
  const table = el('table');
  table.innerHTML = '<thead><tr><th>날짜</th><th>구분</th><th>내용</th><th class="num">수량</th><th class="num">금액</th></tr></thead><tbody></tbody>';
  const tbody = table.querySelector('tbody');
  wrap.appendChild(table);
  ...
  function buildRows(month) {
    ...
    monthInboundRows(month).forEach((r) => {
      const isKeep = r.category === '선결제' || r.vendor === '선결제';
      rows.push({
        sort: `${r.date || ''}-${String(r.id || '')}`,
        date: r.date || '',
        type: isKeep ? 'keep_purchase' : 'inbound',
        typeLabel: isKeep ? '선결제 구매' : '일반 입고',
        detail: `${r.display_name || r.item_name}${r.note ? ` · ${r.note}` : (r.memo ? ` · ${r.memo}` : '')}`,
        qty: `${qtyFmt(r.qty_received)}${r.unit || ''}`,
        amount: `${money(r.total_paid)}원`,
      });
    });

    db.keepHistory
      .filter((h) => (h.date || '').startsWith(month) && h.type === 'ship')
      .forEach((h) => {
        const plan = getKeepPlan(h.key);
        rows.push({
          sort: `${h.date || ''}-${String(h.id || '')}`,
          date: h.date || '',
          type: 'keep_ship',
          typeLabel: '선결제 차감',
          detail: `${plan?.name || h.key}${h.note ? ` · ${h.note}` : ''}`,
          qty: `${qtyFmt(h.qty)}${plan?.unit || ''}`,
          amount: '-',
        });
      });

    Object.keys(db.dailyRecords)
      .filter((d) => d.startsWith(month))
      .sort()
      .forEach((d) => {
        const row = normalizeDailyRow(getDailyRow(d));
        rows.push({
          sort: `${d}-daily`,
          date: d,
          type: 'daily',
          typeLabel: '일일재고 저장',
          detail: `관리환자 ${toNum(row.managed_patients)}명 · 시트 ${toNum(row.sheet_used)}장 · 겔 ${toNum(row.gel_used)}개 · 리바이브 ${toNum(row.revive_used)}개 · 모델링 ${toNum(row.modeling_users)}명`,
          qty: '-',
          amount: '-',
        });
      });
  ...
  function renderRows() {
    ...
    const rows = buildRows(month).filter((r) => type === 'all' ? true : r.type === type);

    tbody.innerHTML = '';
    if (!rows.length) {
      tbody.innerHTML = '<tr><td colspan="5"><div class="empty">선택한 조건의 기록이 없습니다.</div></td></tr>';
      return;
    }

    rows.forEach((r) => {
      const tr = el('tr');
      tr.innerHTML = `<td>${r.date}</td><td>${r.typeLabel}</td><td>${r.detail}</td><td class="num">${r.qty}</td><td class="num">${r.amount}</td>`;
      tbody.appendChild(tr);
    });
  }

  monthInput.onchange = renderRows;
  typeSel.onchange = renderRows;
  renderRows();
  return card;
}
...
function renderReport() {
  ...
  function buildReportHTML(month) {
    ...
    const now = new Date();
    const stamp = `${now.getFullYear()}.${now.getMonth() + 1}.${now.getDate()} ${now.getHours()}:${String(now.getMinutes()).padStart(2, '0')}`;
    const footer = `<div class="print-footer">${stamp}<br>2페이지</div>`;
    return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>${month} 월간보고</title><style>
      @page{size:A4 portrait;margin:12mm}
      body{font-family:ui-sans-serif,system-ui,-apple-system,"Apple SD Gothic Neo","Noto Sans KR",sans-serif;color:#2f2f38;background:#fff;margin:0}
      .page{width:794px;margin:0 auto;padding:0}
      table{width:100%;border-collapse:collapse;margin-top:10px}
      th,td{border:1px solid #e7e3f1;padding:10px 12px;font-size:14px}
      th{background:#f6f3fd;text-align:left}
      .report-block{margin-bottom:18px;border:1px solid #e7e3f1;border-radius:16px;padding:14px;break-inside:avoid;page-break-inside:avoid}
      .num{text-align:right}
      .section-title{margin-bottom:18px}
      .print-footer{margin-top:28px;text-align:right;font-size:12px;color:#777}
    </style></head><body><div class="page">${cloned.innerHTML}${footer}</div></body></html>`;
  }
  htmlBtn.onclick = () => {
    const month = monthInput.value;
  ...
  printBtn.onclick = () => {
    ...
    if (!printWin) return alert('팝업이 차단되어 있습니다. 팝업 허용 후 다시 시도해주세요.');
