...
const pages = ["dashboard","items","daily","inbound","weekly","monthly","orders","report"];
...
function addOrUpdateRecord(kind, record) {
  const list = db.records[kind];
...
function deleteRecord(kind, id) {
  db.records[kind] = db.records[kind].filter((x) => x.id !== id);
  save(db);
}

function renderDaily() {
  $("d-date").value = $("d-date").value || todayStr();
  ...
  for (const r of list) {
    ...
    tr.innerHTML = `
      ...
      <td class="right">${r.err ? `<span class="dangerText">${fmt(r.err)}</span>` : "0"}</td>
  ...
  if (list.length === 0) {
    const tr = document.createElement("tr");
    tr.innerHTML = `<td colspan="8" class="muted">기록 없음</td>`;
    tbody.appendChild(tr);
  }
}

$("d-save-btn").addEventListener("click", () => {
  const date = $("d-date").value;
  const itemId = $("d-item").value;
  const stock = Number($("d-stock").value);
  ...
  const rec = {
    ...
    usage,
    cost,
    unitCostSnapshot, // 잠금 스냅샷
    err,
    memo: "",
    createdAt: Date.now(),
  };

  db.records.daily.unshift(rec);
  save(db);
  $("d-stock").value = "";
  renderDaily();
  renderItems();
...
$("d-tbody").addEventListener("click", (e) => {
  const btn = e.target.closest("button[data-act]");
  if (!btn) return;
  const id = btn.dataset.id;
  if (btn.dataset.act === "del") {
    if (!confirm("해당 일일 기록을 삭제할까요?")) return;
    deleteRecord("daily", id);
    save(db);
    renderDaily();
    renderItems();
    renderDashboard();
