...
const programSections = [
  ...
  },
  {
    ...
    programs:[
      ...
      // 부위/횟수에 따라 금액이 달라 카드(최저가) + 상세(부위별)로 표기
      {
        id:'scar_pico_cross_regen',
        name:'피코프락셀+CROSS+재생',
        price:385000,
        priceLabel:'385,000원~',
        effects:['여드름흉터','모공','피부결'],
        note:'부위/횟수에 따라 금액 상이',
        hideRecommendations:true,
        variants:{
          '양볼': {1:385000, 3:990000, 5:1540000},
          '풀페이스': {1:440000, 3:1210000, 5:1870000}
        },
        detailsNoteLines:[
          '부위/횟수에 따라 금액이 상이합니다.',
          '부분 시술 추가 비용은 적용되지 않습니다.'
        ]
      },
      // 서브시전도 양볼/풀페이스 + 회차별 금액 분리 표기
      {
        id:'scar_subcision',
        ...
        hideRecommendations:true,
        variants:{
          '양볼': {1:550000, 3:1540000, 5:2200000},
          '풀페이스': {1:880000, 3:2200000, 5:3300000}
        },
        detailsNoteLines:[
          '부위/횟수에 따라 금액이 상이합니다.',
          '양볼/풀페이스 가격을 확인 후 회차별로 선택해 주세요.'
        ]
      }
    ]
  },
...
// 프로그램 상세에서 선택한 부위/횟수/옵션 상태 저장
// key: programId -> {variant, sessions, addons:{[name]:boolean}}
...
function renderLeft(){
  ...
  if(view==='price'){
    ...
    filtered.forEach(p=>{
      ...
      const displayPriceText = p.priceLabel ? p.priceLabel : `${fmt(displayPrice)}원`;
      card.innerHTML = `
        <div class="cardRow">
          <div class="cardTitle">${escapeHtml(p.name)}</div>
          <div style="display:flex;gap:8px;align-items:center">
            <button class="smallBtn" data-act="fav" title="즐겨찾기">☆</button>
            <div class="price">${escapeHtml(displayPriceText)}</div>
          </div>
        </div>
...
function renderDetail(){
  ...
  if(selectedItem.type==='program'){
    ...
    const headerPriceText = selectedItem.priceLabel ? selectedItem.priceLabel : `${fmt(livePrice)}원`;

    // variants 기본 선택값
    if(selectedItem.variants && !variantSelections[selectedItem.id]){
      const firstVariant = Object.keys(selectedItem.variants)[0];
      variantSelections[selectedItem.id] = {variant:firstVariant, sessions:1, addons:{}};
    }

    const variantUI = (()=>{
      if(!selectedItem.variants) return '';
      const p = selectedItem;
      const sel = variantSelections[p.id] || {variant:Object.keys(p.variants)[0], sessions:1, addons:{}};
      const blocks = Object.keys(p.variants).map(vName=>{
        const prices = p.variants[vName]||{};
        const rows = [1,3,5].filter(n=>prices[n]!=null).map(n=>`
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:6px 0">
            <button class="smallBtn" data-act="var-select" data-variant="${escapeHtml(vName)}" data-sessions="${n}">${n}회 ${fmt(prices[n])}원</button>
            <button class="smallBtn" data-act="var-add" data-variant="${escapeHtml(vName)}" data-sessions="${n}">추가</button>
          </div>
        `).join('');

        let addonHtml = '';
        if(p.addons && p.addons.eligibleVariant===vName){
          const addonItems = Object.entries(p.addons.items||{});
          if(addonItems.length){
            addonHtml = `
              <div style="margin-top:10px;padding:8px 10px;border:1px solid rgba(255,255,255,.06);border-radius:10px;background:rgba(255,255,255,.02)">
                <div class="muted" style="font-size:11px">추가 부위 (양볼 5회 선택 시 적용)</div>
                ${addonItems.map(([aName,aPrice])=>{
                  const checked = !!(sel.addons && sel.addons[aName]);
                  return `
                    <label style="display:flex;gap:8px;align-items:center;margin:6px 0;color:var(--muted);font-size:12px">
                      <input type="checkbox" data-act="addon-toggle" data-addon="${escapeHtml(aName)}" ${checked?'checked':''}/>
                      <span>${escapeHtml(aName)} <span style="opacity:.78">(+${fmt(aPrice)}원)</span></span>
                    </label>
                  `;
                }).join('')}
              </div>
        ...
        return `
          <div class="kpi" style="margin-top:10px">
            <div style="font-weight:900;margin-bottom:8px">${escapeHtml(vName)}</div>
            ${rows}
            ${addonHtml}
          </div>
        `;
      ...
      const noteLines = (p.detailsNoteLines||[]).map(t=>`<div class="muted">${escapeHtml(t)}</div>`).join('');
      return `<div style="margin-top:6px">${blocks}<div style="margin-top:10px">${noteLines}</div></div>`;
    })();
    ...
    d.innerHTML = `
      <div class="card" style="background:rgba(15,19,32,.35)">
        <div class="cardRow">
          ...
          <div class="price">${escapeHtml(headerPriceText)}</div>
        </div>
        ...
        ${selectedItem.variants ? variantUI : `<div style="line-height:1.7;white-space:pre-line">${escapeHtml(selectedItem.details||'')}</div>`}

        ${(()=>{
          if(selectedItem.hideRecommendations) return '';
    ...
    `;
    qs('#detailAdd').onclick = ()=>{
      if(selectedItem.variants){
        const p = selectedItem;
        const sel = variantSelections[p.id];
        const v = sel?.variant;
        const s = sel?.sessions;
        const base = p.variants?.[v]?.[s];
        if(base==null) return;
        let extra = 0;
        if(p.addons && v===p.addons.eligibleVariant && s===p.addons.eligibleSessions){
          for(const [aName,aPrice] of Object.entries(p.addons.items||{})){
            if(sel.addons?.[aName]) extra += aPrice;
          }
        }
        const finalPrice = base + extra;
        const opt = `${v} ${s}회`;
        const optAddonNames = Object.entries(sel.addons||{}).filter(([,vv])=>vv).map(([k])=>k);
        const optText = optAddonNames.length ? `${opt} / ${optAddonNames.join(', ')}` : opt;
        addToCart(`${p.id}__${v}__${s}${optAddonNames.length?'__addon':''}`, `${p.name} (${optText})`, finalPrice);
      } else {
        addToCart(cartId, cartName, livePrice);
      }
    };

    if(selectedItem.variants){
      d.querySelectorAll('[data-act="var-select"]').forEach(btn=>{
        btn.onclick = ()=>{
          const v = btn.getAttribute('data-variant');
          const s = Number(btn.getAttribute('data-sessions'));
          if(!variantSelections[selectedItem.id]) variantSelections[selectedItem.id] = {variant:v, sessions:s, addons:{}};
          variantSelections[selectedItem.id].variant = v;
          variantSelections[selectedItem.id].sessions = s;
        };
      });
      d.querySelectorAll('[data-act="var-add"]').forEach(btn=>{
        btn.onclick = ()=>{
          const p = selectedItem;
          const v = btn.getAttribute('data-variant');
          const s = Number(btn.getAttribute('data-sessions'));
          const base = p.variants?.[v]?.[s];
          if(base==null) return;
          const sel = variantSelections[p.id] || {variant:v, sessions:s, addons:{}};
          let extra = 0;
          if(p.addons && v===p.addons.eligibleVariant && s===p.addons.eligibleSessions){
            for(const [aName,aPrice] of Object.entries(p.addons.items||{})){
              if(sel.addons?.[aName]) extra += aPrice;
      ...
      d.querySelectorAll('[data-act="addon-toggle"]').forEach(chk=>{
        chk.onchange = ()=>{
          const aName = chk.getAttribute('data-addon');
          if(!variantSelections[selectedItem.id]) return;
          variantSelections[selectedItem.id].addons[aName] = chk.checked;
        };
      });
    }
    ...
    qs('#detailCopy').onclick = ()=>{
      const priceTxt = `${fmt(livePrice)}원`;
