...
/* === Storage Layer: IndexedDB (primary) with localStorage fallback === */
const STORAGE_KEY = "inventory_db";
const IDB_NAME = "inventory_pwa_db";
const IDB_STORE = "kv";
...
async function migrateLocalStorageToIDB() {
  ...
  const fromLS = lsGet(STORAGE_KEY);
  if (!fromLS) return;
  const inIDB = await storageGet(STORAGE_KEY);
  // If IDB empty, migrate once
  if (!inIDB) {
    try { await idbSet(STORAGE_KEY, fromLS); } catch {}
  }
}
...
/* === Patch existing load/save (if present) to use IndexedDB-first storage === */
(async () => {
  try { await migrateLocalStorageToIDB(); } catch {}
  // If the app defines load()/save(db), replace them.
  if (typeof window.load === "function") {
    window._legacyLoad = window.load;
  }
  if (typeof window.save === "function") {
    window._legacySave = window.save;
  }

  // Define async load/save in global scope (some code may call them synchronously; keep sync wrappers too)
})();

async function loadDB() {
  const db = await storageGet(STORAGE_KEY);
  return db;
}
async function saveDB(db) {
  const ok = await storageSet(STORAGE_KEY, db);
  if (!ok) alert("저장 실패: 브라우저 저장공간을 확인하세요.");
  return ok;
}
