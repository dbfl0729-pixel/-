...
function hasPrevQty(recordMap, itemId, date) {
  const dates = Object.keys(recordMap || {}).filter((d) => d < date).sort();
  for (let i = dates.length - 1; i >= 0; i -= 1) {
    const val = recordMap[dates[i]]?.[String(itemId)]?.qty;
...
function getInitialQty(recordMap, itemId, date) {
  const row = recordMap?.[date]?.[String(itemId)] || {};
  const val = row.initial_qty;
  return val === '' || val === undefined || val === null ? 0 : toNum(val);
}
...
function renderDaily() {
  ...
  function getDailyComputed(date, itemId) {
    ...
    const hasPrev = hasPrevQty(db.dailyRecords, itemId, date);
    const initial = getInitialQty(db.dailyRecords, itemId, date);
    const prev = hasPrev ? latestQty(db.dailyRecords, itemId, prevDateISO(date)) : initial;
    ...
    return { prev, inbound, close, actualUse, stored, hasPrev, initial };
  }
  ...
  function renderRows() {
    ...
    if (!items.length) {
      const tr = el('tr');
      tr.innerHTML = '<td colspan="9"><div class="empty">표시할 일일 품목이 없습니다.</div></td>';
      tbody.appendChild(tr);
      return;
    }

    items.forEach((it) => {
      const { prev, inbound, actualUse, stored, hasPrev, initial } = getDailyComputed(d, it.id);
      const initialInput = !hasPrev ? el('input', { type: 'number', value: stored.initial_qty ?? '' }) : null;
      ...
      if (initialInput) tr.children[2].appendChild(initialInput);
      else tr.children[2].innerHTML = '<span class="chip ok">기준 있음</span>';
      tr.children[5].appendChild(qtyInput);
      if (isTarget) tr.children[7].appendChild(useInput);
      else tr.children[7].textContent = '-';

      const syncRow = () => {
        const initialVal = initialInput ? initialInput.value : initial;
        const basePrev = hasPrev ? latestQty(db.dailyRecords, it.id, prevDateISO(d)) : (initialVal === '' ? 0 : toNum(initialVal));
        const closeVal = qtyInput.value;
        const closeNum = closeVal === '' ? 0 : toNum(closeVal);
        const maxStock = basePrev + inbound;
        qtyInput.style.borderColor = (closeVal !== '' && closeNum > maxStock) ? 'rgba(255,107,107,.6)' : 'rgba(255,255,255,.08)';
        const nextActualUse = Math.max(maxStock - closeNum, 0);
        tr.children[3].textContent = money(basePrev);
        tr.children[6].textContent = money(nextActualUse);
        tr.children[8].innerHTML = statusHtml(it.name, nextActualUse, isTarget ? useInput.value : '');
      };

      if (initialInput) initialInput.oninput = syncRow;
  ...
  saveBtn.onclick = () => {
    ...
    for (let idx = 0; idx < items.length; idx += 1) {
      const it = items[idx];
      const tr = rows[idx];
      const existing = db.dailyRecords[d][String(it.id)] || {};
      const initialInput = tr.children[2].querySelector('input');
      const qty = tr.children[5].querySelector('input').value;
      const useCell = tr.children[7];
      const useInput = useCell.querySelector('input');
      const useQty = useInput ? useInput.value : '';
      const hasPrev = hasPrevQty(db.dailyRecords, it.id, d);
      const initialVal = initialInput ? initialInput.value : existing.initial_qty;
      const prevBase = hasPrev ? latestQty(db.dailyRecords, it.id, prevDateISO(d)) : (initialVal === '' || initialVal === undefined ? 0 : toNum(initialVal));
      const inb = sumInbound(it.id, d);
      const closeNum = qty === '' ? 0 : toNum(qty);

      if (!hasPrev && (initialVal === '' || initialVal === undefined || initialVal === null)) {
        alert(`저장 불가: ${it.name} 첫 사용일이면 초기 재고를 입력해야 합니다.`);
        return;
      }
      if (qty !== '' && closeNum > (prevBase + inb)) {
        alert(`저장 불가: ${it.name} 퇴근 재고가 (기준재고+입고)보다 큽니다.`);
        return;
      }
      db.dailyRecords[d][String(it.id)] = {
        ...existing,
        initial_qty: !hasPrev ? (initialVal === '' ? '' : toNum(initialVal)) : (existing.initial_qty ?? ''),
        qty: qty === '' ? '' : closeNum,
  ...
  bindSavedSelect(savedSel);
  renderRows();
  card.appendChild(el('div', { class: 'mini', html: '첫 사용일에는 초기 재고를 직접 입력합니다. 이후부터는 전일 재고가 자동으로 이어집니다. 당일 총 환자수는 전체 관리 환자 수이고, 사용 인원은 시트팩·겔시트팩·리바이브팩을 실제 사용한 사람 수만 입력합니다.' }));
  return card;
}
...
function keepRemain(key) {
  const cfg = KEEP_CONFIG[key];
  const state = db.keep?.[key] || {};
  const tier = cfg.tiers.find((x) => x.tier === state.tier) || cfg.tiers[0];
  const shipped = toNum(state.shipped_total);
  return {
    total: toNum(tier.total_qty),
    shipped,
    remain: Math.max(toNum(tier.total_qty) - shipped, 0),
    unit_cost: toNum(tier.unit_cost),
    unit: cfg.unit,
    tier: tier.tier,
  };
...
function latestQtyInMonth(itemId, ym) {
  const lastDay = `${ym}-31`;
  return latestQty(db.dailyRecords, itemId, lastDay);
}

function computeMonthlyUse(item, ym) {
  let monthlyUse = 0;
  Object.keys(db.dailyRecords).filter((d) => d.startsWith(ym)).forEach((d) => {
    const stored = db.dailyRecords[d]?.[String(item.id)] || {};
    if (PER_PATIENT_TARGETS.has(item.name)) {
      monthlyUse += toNum(stored.use_qty);
    } else {
      const hasPrev = hasPrevQty(db.dailyRecords, item.id, d);
      const prev = hasPrev ? latestQty(db.dailyRecords, item.id, prevDateISO(d)) : getInitialQty(db.dailyRecords, item.id, d);
      const inbound = sumInbound(item.id, d);
      const close = stored.qty === '' || stored.qty === undefined || stored.qty === null ? 0 : toNum(stored.qty);
...
function renderMonthlyReport() {
  const card = el('div', { class: 'card' });
  card.appendChild(el('h2', { html: '월간 보고 (원장님 보고용 요약 포함)' }));
  ...
  const summaryKpi = el('div', { class: 'kpi' });
  card.appendChild(summaryKpi);

  const emptyBox = el('div', { class: 'empty', style: { display: 'none', marginTop: '12px' } });
  card.appendChild(emptyBox);

  const wrap = el('div', { class: 'table-wrap', style: { marginTop: '12px' } });
  const table = el('table');
  ...
  const lowWrap = el('div', { class: 'table-wrap', style: { marginTop: '12px', display: 'none' } });
  const lowTable = el('table');
  lowTable.innerHTML = '<thead><tr><th>부족 주의 품목</th><th class="num">최신 재고</th><th>기준</th></tr></thead><tbody></tbody>';
  lowWrap.appendChild(lowTable);
  card.appendChild(lowWrap);
  const lowTbody = lowTable.querySelector('tbody');
  ...
  function renderRows() {
    ...
    const patients = Object.keys(db.patients).filter((d) => d.startsWith(ym)).reduce((a, d) => a + toNum(db.patients[d]), 0);
    const dailyDates = Object.keys(db.dailyRecords).filter((d) => d.startsWith(ym));
    const dailySaves = dailyDates.length;
    const inboundRows = db.inbound.filter((r) => r.date.startsWith(ym));
    ...
    const keepSheet = keepRemain('sheetmask');
    const keepModel = keepRemain('modeling');
    const visibleDailyItems = getVisibleItemsByCycle('일일');

    const hasAnyData = patients > 0 || dailySaves > 0 || inboundRows.length > 0 || coupangChecked > 0;
    emptyBox.style.display = hasAnyData ? 'none' : 'block';
    wrap.style.display = hasAnyData ? 'block' : 'none';
    lowWrap.style.display = 'none';

    if (!hasAnyData) {
      emptyBox.textContent = '해당 월 데이터가 없습니다. 일일 재고 저장, 당일 환자수 입력, 입고 기록 중 1개 이상을 먼저 등록하세요.';
      return;
    }

    let totalDailyUse = 0;
    const lowItems = [];
    visibleDailyItems.forEach((it) => {
      const monthlyUse = computeMonthlyUse(it, ym);
      totalDailyUse += monthlyUse;
      const latestClose = latestQtyInMonth(it.id, ym);
      if (latestClose <= 5) {
        lowItems.push({ name: it.name, qty: latestClose, unit: it.base_unit });
      }
    });

    const avgUsePerPatient = patients > 0 ? (totalDailyUse / patients).toFixed(2) : '-';

    summaryKpi.appendChild(kpiBox('월 환자수', money(patients)));
    summaryKpi.appendChild(kpiBox('월 입고 건수', money(inboundRows.length)));
    summaryKpi.appendChild(kpiBox('일일품목 총 사용량', money(totalDailyUse)));
    summaryKpi.appendChild(kpiBox('재고 주의 품목 수', money(lowItems.length)));
    summaryKpi.appendChild(kpiBox('환자 1인당 평균 사용량', avgUsePerPatient));
    summaryKpi.appendChild(kpiBox('입고 총금액', `${money(inboundCost)}원`));
    ...
    tbody.appendChild(row('월 입고 건수', money(inboundRows.length), '선택 월 입고 등록 횟수'));
    ...
    tbody.appendChild(row('(더마소드) 겔시트팩 현재 킵 잔량', `${money(keepSheet.remain)}${keepSheet.unit}`, '계약총량 - 누적출고 기준 현재 잔량'));
    tbody.appendChild(row('(하라셀) 모델링팩 현재 킵 잔량', `${money(keepModel.remain)}${keepModel.unit}`, '계약총량 - 누적출고 기준 현재 잔량'));
    tbody.appendChild(row('일일품목 총 사용량', money(totalDailyUse), '일일 재고 기준 사용량 합계'));
    tbody.appendChild(row('환자 1인당 평균 사용량', avgUsePerPatient, patients > 0 ? '일일품목 총 사용량 ÷ 월 환자수' : '환자수 0으로 계산 불가'));
    tbody.appendChild(row('재고 주의 품목 수', money(lowItems.length), '최신 일일 재고가 5 이하인 품목 수'));
    ...
    if (lowItems.length) {
      ...
      lowItems.sort((a, b) => a.qty - b.qty || a.name.localeCompare(b.name, 'ko'));
      lowItems.forEach((item) => {
        const tr = el('tr');
        tr.innerHTML = `<td>${item.name}</td><td class="num">${money(item.qty)}${item.unit}</td><td>최신 일일 재고 5 이하</td>`;
