...
function renderPrepaid() {
  ...
  function makeKeepCard(key) {
    ...
    db.keep[key] = { ...st, tier: st.tier || cfg.tiers[0].tier, shipped_total: toNum(st.shipped_total) };
    ...
    tierSel.value = db.keep[key].tier;
    ...
    function refresh() {
      db.keep[key].tier = tierSel.value;
    ...
    btn.onclick = () => {
      ...
      db.keep[key].shipped_total = shipped + q;
      ...
      save();
      alert('출고요청 완료: 누적 출고 증가 + 업체 잔량 자동 차감 + 입고 자동 생성');
      refresh();
    };

    const box = el('div', { class: 'card', style: 'background:rgba(17,26,44,.45)' });
    box.appendChild(el('div', { html: `<b>${cfg.name}</b>` }));
    box.appendChild(el('div', { class: 'mini', html:
      key === 'sheetmask'
        ? '계약 티어는 300장/500장 중 1회 선택 후 필요할 때마다 10장씩 출고요청합니다.'
        : '모델링은 100kg 선결제 고정이며 필요할 때마다 1kg씩 출고요청합니다.'
    }));
  ...
  card.appendChild(el('div', { class: 'mini', html: "시트팩은 300장/500장 선결제만 가능하고 10장 단위로 출고합니다. 모델링은 100kg 선결제만 가능하고 1kg 단위로 출고합니다. 업체 잔량은 계약 총량 - 누적 출고량으로 자동 계산되며, 출고요청 시 입고 기록이 자동 생성됩니다." }));
  return card;
}
